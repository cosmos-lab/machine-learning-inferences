...
def generate_answer(query, context_lines):
    context_text = "\n".join(context_lines)
    prompt = f"""Answer the question clearly using ONLY the facts below.
Context:
{context_text}
...
"""
    ...
    with torch.no_grad():
        outputs = model.generate(**inputs, max_new_tokens=MAX_NEW_TOKENS, do_sample=False, num_beams=4, top_p=0.9, temperature=0.7, early_stopping=True)
